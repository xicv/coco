...
5. Repeat: `coco_goal_status({ repoDir, goalId })` → act on `nextAction`:
   ...
   - **`review`** → `coco_goal_op_start({ phase:"review", kind:"oracle" })`, then `consult` (Oracle, review mode, `engine:"browser"`) on the committed diff; instruct Oracle to finish with `VERDICT: clean` or `VERDICT: blocking`. `coco_goal_record({ phase:"review", expectedSha:<HEAD>, evidence:"<oracle session>", reviewOutput:"<oracle's full text>" })`. If unavailable/ambiguous → STOP (rule 5).
